...
def perfect_square(score):
    if score == 1:
        return True
    for i in range(int(score/2), 1, -1):
        if i * i == score:
            return True
    return False
...
def make_averaged(original_function, total_samples=1000):
    ...
    """
    # BEGIN PROBLEM
    def averaged_roll_dice(*arg):
        sum = 0
        for i in range(0, total_samples):
            sum += original_function(*arg)
        ans = sum / total_samples
        return ans
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    ans1 = square_update(0, score, opponent_score, dice=six_sided)
    if ans1 - score >= threshold:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
