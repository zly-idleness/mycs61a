...
def num_eights(pos):
    ...
    if pos < 8:
        ...
        if pos % 10 == 8:
            return 1 + num_eights(int(pos / 10))
        else:
            return num_eights(int(pos / 10))
    ...
    def find_cnt(x):
        if x < 8:
            ...
            if num_eights(x) > 0 or x % 8 == 0:
                return 1 + find_cnt(x - 1)
            else:
                return find_cnt(int(x - 1))
    if n == 1:
        return 1
    else:
        return (-1)**(find_cnt(n - 1)) + pingpong(n-1)
...
def count_coins(change):
    ...
    """
    "*** YOUR CODE HERE ***"
    def Max_count_coins(change, max):
        if change == 0:
            return 1
        if change < 0:
            return 0
        if max == None:
            ...
            part1 = Max_count_coins(change - max, max)
            part2 = Max_count_coins(change, next_smaller_coin(max))
        return part1 + part2
    return Max_count_coins(change, 25)
